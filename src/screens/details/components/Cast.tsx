  ...
  return (
    <>
      ...
      <m.div
        ...
        initial="hidden"
        animate="show"
        className="sm:-mt-2 xs:-mt-[6px] -mt-1 flex flex-wrap gap-x-2 gap-y-2 sm:gap-[14px] md:gap-x-4 md:gap-y-4"
      >
        {topCasts.map((actor) => {
          return (
            <m.figure
              variants={fadeDown}
              key={actor.id}
              className="flex flex-col justify-start gap-x-2 gap-y-2"
            >
              ...
              <h4 className="sm:-mt-0 max-w-[40px] text-center font-semibold text-[10px] text-mode-300 leading-snug sm:text-[10.75px] md:max-w-[64px] md:text-[12px]">
                {actor.name}
              </h4>
            </m.figure>
          )
